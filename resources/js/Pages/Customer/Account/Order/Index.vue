...
function filterOrders(value){
    router.get(route('customer.account.order.index'), 
    {
        ...(value != '' && { status: value }),
    },
    {
                ...
                <div class="divide-y divide-gray-200 dark:divide-gray-700" v-if="orders.data.length">
                    <div class="flex flex-wrap items-center gap-y-4 py-6" v-for="order in orders.data" :key="order.id">
                        ...
                        <div class="w-full grid sm:grid-cols-2 lg:flex lg:w-64 lg:items-center lg:justify-end gap-4">
                            <button v-if="order.status == 'unpaid'" @click="cancelOrder(order.id)" type="button" class="w-full rounded-lg border border-red-700 px-3 py-2 text-center text-sm font-medium text-red-700 hover:bg-red-700 hover:text-white focus:outline-none focus:ring-4 focus:ring-red-300 dark:border-red-500 dark:text-red-500 dark:hover:bg-red-600 dark:hover:text-white dark:focus:ring-red-900 lg:w-auto">
                                Cancel order
                            </button>
                            <Link v-if="order.status == 'delivered' || order.status == 'cancelled'" :href="route('customer.cart.orderAgain', order.id)" as="button" method="post" class="w-full rounded-lg bg-primary-700 px-3 py-2 text-sm font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 lg:w-auto">
                                Order again
                            </Link>
